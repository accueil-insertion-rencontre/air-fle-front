...
import { TodoTask, Subtask, TodolistService } from '../../services/todolist.service';
...
export class TodoItemComponent {
  ...
  @Output() taskUpdated = new EventEmitter<{subtaskId: string, updatedSubtask: TodoTask | Subtask}>();
  ...
  toggleSubtaskStatus() {
    ...
    console.log('🔄 Toggling subtask status for:', this.task.id, 'current status:', this.task.status);
  ...
  toggleExpand() {
    if (!this.isSubTask && 'subtasks' in this.task && this.task.subtasks && this.task.subtasks.length > 0) {
      this.isExpanded = !this.isExpanded;
    }
  }

  hasSubtasks(): boolean {
    return !this.isSubTask && 'subtasks' in this.task && Boolean(this.task.subtasks && this.task.subtasks.length > 0);
  }
