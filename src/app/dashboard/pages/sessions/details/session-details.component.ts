...
export class SessionDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.params.subscribe(params => {
      if (params['id']) {
        console.log('ID de session reçu dans la route:', params['id'], 'type:', typeof params['id']);
  ...
  loadSession(): void {
    console.log('loadSession appelée avec sessionId:', this.sessionId, 'type:', typeof this.sessionId);
  ...
  calculateDuration(): number {
    if (!this.session?.started_at || !this.session?.finished_at) return 0;
    
    const startDate = new Date(this.session.started_at);
    const endDate = new Date(this.session.finished_at);
  ...
  onSubmitGroup(): void {
    ...
    // Préparer les données avec la session pré-sélectionnée
    const groupData = {
      label: this.groupForm.value.label,
      session_id: this.sessionId
    };
    
    console.log('Données de groupe à créer:', groupData);
    ...
    this.groupService.createGroup(groupData).subscribe({
      ...
      error: (error) => {
        ...
        this.error = error?.error?.message || error?.message || 'Une erreur est survenue lors de la création du groupe';
        this.groupLoading = false;
      }
    });
  }
  ...
  deleteSession(): void {
    ...
    this.alertService.confirm(
      `Êtes-vous sûr de vouloir supprimer la session "${this.session.label}" ?\n\n` +
